...
def select_shengutong_daima(conn):
    sql='select  distinct dai_ma,name from shang_zheng_gushi.shang_gutong order by dai_ma desc;'
    cur=conn.cursor()
    cur.execute(sql)
...
def download_gupiao(gupiao_list,conn):
    sql='DROP TABLE IF EXISTS `%s`;CREATE TABLE `shang_zheng_gushi`.`%s` (`id` INT NOT NULL AUTO_INCREMENT,`date` DATE NOT NULL,`open` FLOAT NOT NULL,`close` FLOAT NOT NULL,`high` FLOAT NOT NULL,`low` FLOAT NOT NULL,`volume` FLOAT NOT NULL,PRIMARY KEY (`id`));'
    sql_1='INSERT INTO `shang_zheng_gushi`.`%s` (`date`, `open`, `close`, `high`, `low`, `volume`) VALUES (%s,%s,%s,%s,%s,%s);'
    cur=conn.cursor()

    for gupiao in gupiao_list:
            hist_data=ts.get_k_data(gupiao[0],start='2014-01-01')
...
def download_gupiao_houxu(gupiao_list,conn):
    cur=conn.cursor()
    sql_4='SELECT max(date) FROM shang_zheng_gushi.`%s`;'
    ...
    for gupiao in gupiao_list:
        ...
        # 判断是否存在该表
        sql='''select TABLE_NAME from INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA='shang_zheng_gushi' and TABLE_NAME="%s";'''
        ...
        # 该表不存在
            ...
            # 不存在该表就新建
            sql_2='create table if not exists `shang_zheng_gushi`.`%s`(`id` INT NOT NULL AUTO_INCREMENT,`date` DATE NOT NULL,`open` FLOAT NOT NULL,`close` FLOAT NOT NULL,`high` FLOAT NOT NULL,`low` FLOAT NOT NULL,`volume` FLOAT NOT NULL,PRIMARY KEY (`id`));'
            cur=conn.cursor()
            cur.execute(sql_2,(gupiao[0]))
            conn.commit()#提交事务
            sql_3='INSERT INTO `shang_zheng_gushi`.`%s` (`date`, `open`, `close`, `high`, `low`, `volume`) VALUES (%s,%s,%s,%s,%s,%s);'
            hist_data=ts.get_k_data(gupiao[0],start='2015-1-1')
            for datas,row in hist_data.iterrows():
...
def select_jiaoyi_day(conn):
    sql="SELECT date FROM shang_zheng_gushi.`'sh'`;"
    cur=conn.cursor()
    cur.execute(sql)
...
def select_gupiao_money(gupiao_daima,quchu_day,conn):
    sql='SELECT date,jing_er,mai_ru_er,mai_chu_er,chengjiao_er,zhang_die_fu_percent,name FROM shang_zheng_gushi.shang_gutong where dai_ma=%s;'
    cur=conn.cursor()
...
def zhiding_day(gupiao_daima,gupiao_money,tian_shu,print_or_not,conn):
    ...
    sql="SELECT id,close FROM shang_zheng_gushi.`%s` where date=%s;"
    sql_1="SELECT close,date FROM shang_zheng_gushi.`%s`where id=%s;"
    cur=conn.cursor()
